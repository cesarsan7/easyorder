            ...
            <div>
              ...
              <div className="border-t border-gray-100 mt-4 pt-3 space-y-1.5">
                ...
                <div className="flex justify-between text-base font-bold text-gray-900">
                  <span>Total</span>
                  <span style={{ color: '#E63946' }}>{fmt(detail.total)}</span>
                </div>
              </div>
            </div>
          ) : (
...
export default function DashboardPage() {
  ...
  async function handleStatusChange(id: number, estado: string) {
    ...
    try {
      ...
      // Optimistic update — replace the order's estado locally
      ...
      setOrders((prev) =>
        prev
          ...
          // Remove from active view if moved to terminal state and filter is ''
          .filter((o) => {
            if (filter !== '') return true
            return !['entregado', 'cancelado'].includes(o.estado)
          })
      )
      // Keep panel in sync if it's open for this order
      setSelectedOrder((prev) =>
        prev?.id === updated.id ? { ...prev, estado: updated.estado, updated_at: updated.updated_at } : prev
      )
    } catch {
      // Silent fail — next poll will correct state
    } finally {
      setUpdatingId(null)
    }
  }
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <p className="text-sm text-gray-500">
              {filter ? `No hay pedidos con estado "${activeTab.label}"` : 'No hay pedidos activos por ahora'}
            </p>
          </div>
        ) : (
          <>
            <p className="text-xs text-gray-400 mb-4">{total} pedido{total !== 1 ? 's' : ''}</p>
