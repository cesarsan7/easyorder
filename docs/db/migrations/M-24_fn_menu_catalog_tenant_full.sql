-- ──────────────────────────────────────────────────────────────────────────────
-- M-24: Completar fn_menu_catalog(integer) y fn_menu_lookup(text, integer)
--       con las columnas menu_item_id, menu_variant_id, extras_json
--
-- Problema:
--   M-22 creó los overloads multi-tenant de fn_menu_catalog y fn_menu_lookup
--   pero omitió las columnas que M-12 había agregado a las versiones originales:
--     menu_item_id, menu_variant_id, extras_json
--   n8n Apertura (consultar_menu) consulta esas columnas → "column does not exist"
--
-- Solución:
--   DROP + CREATE de los dos overloads con p_restaurante_id para añadir
--   las 3 columnas faltantes. El overload original sin restaurante_id
--   (fn_menu_catalog() y fn_menu_lookup(text)) NO se toca.
-- ──────────────────────────────────────────────────────────────────────────────

BEGIN;

-- DROP primero porque PostgreSQL no permite cambiar RETURNS con CREATE OR REPLACE
DROP FUNCTION IF EXISTS public.fn_menu_lookup(text, integer);
DROP FUNCTION IF EXISTS public.fn_menu_catalog(integer);

-- ── 1. fn_menu_catalog(integer) ───────────────────────────────────────────────

CREATE FUNCTION public.fn_menu_catalog(
...
-- ── 2. fn_menu_lookup(text, integer) ─────────────────────────────────────────

CREATE FUNCTION public.fn_menu_lookup(
...
-- Verificación:
--
-- SELECT categoria, producto, menu_item_id, menu_variant_id,
--        extras_json IS NOT NULL AS tiene_extras
--   FROM public.fn_menu_catalog(1) LIMIT 5;
--
-- SELECT categoria, producto, menu_item_id
--   FROM public.fn_menu_lookup(NULL, 1) LIMIT 5;
-- ──────────────────────────────────────────────────────────────────────────────
